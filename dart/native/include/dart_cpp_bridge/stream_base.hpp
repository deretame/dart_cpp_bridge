...
// Coroutine bodies are stdexec::task (which is itself a sender), so a Stream can
...
//
// This header holds the core Stream type only (no dependency on the runtime
// singleton). `Stream::interval` is declared here but defined in stream.hpp,
// which pulls in dart_cpp_bridge/runtime.hpp for dcb::sleep — an auto-return
// sender must be fully defined at its call site (clang rejects the
...
struct StreamImpl {
  ...
  virtual stdexec::task<std::optional<T>> next() = 0;
};
...
class Stream {
 public:
  ...
  stdexec::task<std::optional<T>> next()
  {
    if (!impl_) {
  ...
  // Stream from an existing vector of values.
  static Stream<T> from_vector(std::vector<T> values)
  {
    struct Impl : StreamImpl<T> {
      ...
      explicit Impl(std::vector<T> v) : items(std::move(v)) {}
  ...
  // values are 0, 1, 2, ...
  // Defined in stream.hpp (needs dcb::sleep, which requires the Runtime
  ...
  auto map(F&& f) &&->Stream<std::invoke_result_t<F, T>>
  {
    ...
    struct Impl : StreamImpl<U> {
      ...
      stdexec::task<std::optional<U>> next() override
      {
  ...
  Stream<Acc> scan(Acc init, F&& f) &&
  {
    struct Impl : StreamImpl<Acc> {
      ...
      stdexec::task<std::optional<Acc>> next() override
      {
        auto v = co_await upstream.next();
        if (!v) {
          co_return std::nullopt;
  ...
  Stream<std::pair<T, U>> zip(Stream<U> other) &&
  {
    struct Impl : StreamImpl<std::pair<T, U>> {
      ...
      stdexec::task<std::optional<std::pair<T, U>>> next() override
      {
  ...
  stdexec::task<std::vector<T>> collect()
  {
    std::vector<T> out;
  ...
  template <typename F>
  stdexec::task<void> for_each(F&& f)
  {
    while (auto v = co_await next()) {
      f(std::move(*v));
    }
  ...
  stdexec::task<std::optional<T>> first() { co_return co_await next(); }

  stdexec::task<std::size_t> count()
  {
    std::size_t n = 0;
    while (auto v = co_await next()) {
      (void)v;
  ...
  template <typename Acc, typename F>
  stdexec::task<Acc> fold(Acc init, F&& f)
  {
