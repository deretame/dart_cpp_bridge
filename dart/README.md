...
The package uses the stdexec-based runtime. It contains a broad C++
async/concurrency migration from the legacy async-simple model. The Dart
API, C ABI, wire protocol, generated file layout, and method-ID contracts remain
stable, but C++ async business code is not source-compatible with the legacy
runtime. See the
[version guide](https://github.com/deretame/dart_cpp_bridge/blob/main/docs/versioning.md)
before copying C++ async examples or migrating existing code.
...
- Regenerate bindings with the matching `dcb_gen_tool` release; do not mix
  legacy generated native code with current runtime headers or tooling.
