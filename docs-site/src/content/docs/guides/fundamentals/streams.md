...
- A new call creates a fresh stream with a new stream id, so cancel-then-resubscribe works
  (covered by the codegen_demo stress tests).

## Threads and lifetime

- `add()` / `end()` / `error()` are thread-safe and can be called from the io thread, a pool
  thread, or a foreign event-loop thread (see `uv_stream` / `worker_stream` in codegen_demo,
...
- [Choosing a Marker](/dart_cpp_bridge/guides/fundamentals/markers/)
