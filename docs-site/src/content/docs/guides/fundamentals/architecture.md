...
Business code can use the utilities Runtime provides directly: `spawn`, `spawn_blocking`, `spawn_detached`, `channel`, `sleep`, etc. See [Basic Runtime](/dart_cpp_bridge/guides/fundamentals/runtime/).
...
The low-level binary frame format is documented in [Wire Protocol](/dart_cpp_bridge/reference/wire-protocol/). `ByteReader` / `ByteWriter` handle encoding/decoding of primitive types, strings, containers, optionals, and data classes inside the payload. Both sides must read and write in exactly the same order.
...
The bridge supports integrating non-asio event loops (libuv, glib, custom loops, etc.) into the coroutine system through the `ForeignExecutor` adapter, enabling cross-runtime non-blocking communication and Dart callback invocation. See [Foreign Runtime Integration](/dart_cpp_bridge/guides/advanced/foreign-runtime/).

For pure C code or scenarios that do not depend on async-simple, the [Pure C Bridge API](/dart_cpp_bridge/guides/advanced/cbridge/) is provided (callback style, zero C++ dependency).
