...
This guide follows the current stdexec-based repository. Published 1.x projects
should use the [version guide](/dart_cpp_bridge/versions/) and keep their v1
async-simple headers and generated output together.
:::
...
- **Dart SDK >= 3.10.0**
- **Git** (needed when using the default CMake dependency path that fetches
  Asio and stdexec; host-provided dependencies can use a package manager or
  an existing monorepo setup instead)
