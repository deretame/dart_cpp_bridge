...
:::note[v2.1.0]
本指南按当前 stdexec 仓库实现编写。已发布的 1.x 项目请先查看
[版本说明](/dart_cpp_bridge/zh-cn/versions/)，并让 v1 的 async-simple 头文件
与生成物保持成套。
:::
...
- **Dart SDK >= 3.10.0**
- **Git**（使用默认 CMake 依赖路径、需要拉取 Asio 和 stdexec 时必需；如果
  由宿主工程提供依赖，则可以改用包管理器或已有的 monorepo 配置）
