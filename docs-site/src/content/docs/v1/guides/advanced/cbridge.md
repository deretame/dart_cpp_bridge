...
description: C99-compatible API for pure C callers — encoding/decoding, async
  operations, and calling Dart functions
slug: v1/guides/advanced/cbridge
---
...
### Comparison with spawn\_blocking

| | `dcb_async_*` | `spawn_blocking` |
|--|---|---|
| Occupies thread? | No (coroutine suspends) | Yes (occupies a pool thread) |
| Concurrency limit | None (only consumes one op\_id) | thread\_pool size (default 4) |
