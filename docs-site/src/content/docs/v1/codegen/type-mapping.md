...
dart\_cpp\_bridge's wire protocol is not zero-copy itself; frame data needs serialization/deserialization between Dart and C++. However, for everyday types (basic types, small data classes, short strings), this copy overhead is negligible and generally not a concern.
