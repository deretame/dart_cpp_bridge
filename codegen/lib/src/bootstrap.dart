...
Future<BootstrapResult> bootstrap(
  ...
  // 5a. Windows: ensure VC runtime DLLs are present next to python.exe.
  if (platform.isWindows) {
    _ensureVcRuntime(pythonExe, log);
  }
...
/// On Windows, ensure VC runtime DLLs are present next to python.exe.
///
/// The `install_only_stripped` python-build-standalone archive ships with
/// older vcruntime DLLs that may be incompatible with libclang-ng's
/// libclang.dll. Search for the newest VC redistributable CRT on the system
/// and copy those DLLs into the python directory.
void _ensureVcRuntime(String pythonExe, _Logger log) {
  final exeDir = p.dirname(pythonExe);
  const dlls = ['vcruntime140.dll', 'vcruntime140_1.dll', 'msvcp140.dll'];

  final crtDir = _findVcRedistCrt();
  if (crtDir != null) {
    for (final dll in dlls) {
      final source = File(p.join(crtDir, dll));
      if (source.existsSync()) {
        try {
          source.copySync(p.join(exeDir, dll));
        } catch (_) {}
      }
    }
    return;
  ...
  for (final dll in dlls) {
    final target = File(p.join(exeDir, dll));
    final source = File(p.join(systemDir, dll));
    if (source.existsSync()) {
      try {
      ...
      log.info(
          'WARNING: $dll not found. Install "Microsoft Visual C++ '
          'Redistributable" (vcredist 2015-2022).');
    }
  }
}

/// Search for the Visual C++ redistributable CRT directory (x64).
///
/// Looks in standard Visual Studio installation paths for the newest
/// `Microsoft.VC14x.CRT` folder containing vcruntime140.dll.
String? _findVcRedistCrt() {
  const roots = [
  ...
  for (final root in roots) {
    ...
    // Pattern: <root>/<version>/<edition>/VC/Redist/MSVC/<msvc_ver>/x64/Microsoft.VC*.CRT/
    try {
      for (final vsDir in rootDir.listSync()) {
        if (vsDir is! Directory) continue;
        for (final editionDir in vsDir.listSync()) {
          if (editionDir is! Directory) continue;
          final redistBase = Directory(
              p.join(editionDir.path, 'VC', 'Redist', 'MSVC'));
          if (!redistBase.existsSync()) continue;

          for (final msvcDir in redistBase.listSync()) {
            if (msvcDir is! Directory) continue;
            final x64Dir = Directory(p.join(msvcDir.path, 'x64'));
            if (!x64Dir.existsSync()) continue;

            for (final crtDir in x64Dir.listSync()) {
              if (crtDir is! Directory) continue;
              final name = p.basename(crtDir.path);
              if (!name.startsWith('Microsoft.VC') ||
                  !name.endsWith('.CRT')) {
                continue;
              }
              final dll = File(p.join(crtDir.path, 'vcruntime140.dll'));
              if (dll.existsSync()) {
                best = crtDir.path; // Take the last (newest) found.
              }
            }
          }
        }
