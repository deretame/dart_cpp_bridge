...
> （P2300 参考实现，锁 `nvhpc-26.05` tag，MSVC 19.51 / VS2026）。以下问题均为
...
- **衍生坑**：`defaulted operator==` 会逐成员比较，`asio::io_context` 没有
...
- **现象**：`then`/`let_value` 等适配器组合自定义 sender 时，`__sync_attrs` 查询
...
- **根因**：sexpr 的 opstate 是 immovable（`STDEXEC_IMMOVABLE`）；`emplace`/
...
| 问题 | 说明 |
|------|------|
| `stdexec::start` 要求 **lvalue** opstate | `start_t::operator()(_Op&)`；`start(connect(...))` 临时对象编译失败，必须 `auto op = connect(...); start(op);` 两步 |
...
| 模板参数顺序 | `run_async<T, S, Encode>` 显式给 T 时 S 必须可推导，写反会全部 C2672 |
...
- `typeid(T).name()` 打印 sender 类型，对比 `connect_result_t` 与
...
- **对策**：**手写 `on_scheduler_sender` 包装**（connect child + inner receiver +
  `asio::post` 回目标调度器），只依赖 `connect`/`set_value`/`start` 稳定 API，
  运行期验证通过。`continues_on` 从 runtime 中移除。

### 15.2 `exec::task`（协程）与自定义 env 不兼容
...
- **对策**：smoke 全部改 receiver/sender 组合（不再用协程）；`stream.hpp` 的
  `exec::task` 接口保留但**未被实例化**（未使用则不报错）——后续若要用协程，
  需升级 stdexec 版本或换实现。
...
| `exec::task` 协程 | ✗ 弃用（签名计算失败） |
| `stdexec::just` / `then` / `write_env` / `sync_wait` / `stop_token` | ✓ 正常 |
...
- **根因**：P2300 要求 opstate 在 `start()` 后必须存活到完成信号发出；局部
  `auto op = connect(...)` 在函数（如 io 上执行的 factory）返回时析构，而
  业务 sender（DartFn）尚未完成。
- **修复**：`dcb::start_detached(sndr, rcvr)` —— 堆分配 `faf_state`
