...
`dart_cpp_bridge` 目前存在两代 C++ 异步模型。v1 继续为已有 1.x 项目保留，
当前基于 stdexec 的 v2.1.0 已正式发布。
...
| **v2** | 已发布 2.1.0 | stdexec sender、scheduler、`stdexec::task`、stop token | 新项目和已迁移的 v2 项目 |
...
v2 已作为 `dart_cpp_bridge: ^2.1.0` 和 `dcb_gen_tool: ^2.1.0` 发布。两个包的
版本必须保持一致，并使用匹配版本的工具重新生成代码。
