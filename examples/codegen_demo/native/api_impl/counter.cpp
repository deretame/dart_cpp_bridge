...
exec::task<std::int32_t> Counter::value() const { co_return value_; }
...
exec::task<void> Counter::increment(std::int32_t delta) {
  value_ += delta;
  co_return;
}
...
exec::task<std::int32_t> Counter::addList(
...
exec::task<void> Counter::setValue(
...
exec::task<Counter> Counter::duplicate() const {
  co_return Counter(value_);
}

exec::task<std::int32_t> Counter::addTo(const Counter& other) {
  value_ += other.value_;
  co_return value_;
}
...
exec::task<std::string> Counter::greetDartFn(
...
exec::task<std::int32_t> addCounters(const Counter& a,
                                                   const Counter& b) {
