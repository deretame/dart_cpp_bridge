...
- Business functions are `exec::task` coroutines composed with
  `stdexec::starts_on(worker.scheduler(), ...)`; the exec::task reschedules
...
- The wire dispatch (`native/generated/wire_dispatch.cpp`) launches async
  methods as `exec::task` coroutines on the io thread
...
│   ├── api/foreign_api.h # BRIDGE_* API (exec::task signatures)
